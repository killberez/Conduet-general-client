...
const SelectionRadio = styled.div<RadioSelection>`
  width: 6px;
  height: 6px;
...
const RadioSelectionList =  (props: Props) => {
  const [isOpen, setIsOpen] = useState(false);
  const list = props.list.map((d: KeyList, indx: number) => {
    ...
    return (
      <List
        ...
        onClick={() => {
          if (!d.disabled) {
            props.onSelect(d.keyName, d.type, d.parentKey);
          }
        }}
      >
        <SelectionDiv>
          <SelectionRadio
            selected={d.keyName === props.selectedValue ? true : false}
          />
        </SelectionDiv>
        <Key>
          {keyName.substr(-2) === "[]"
